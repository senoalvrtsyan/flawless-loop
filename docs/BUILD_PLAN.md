...
| [ ] | **B10b** | **Client subscribes**, merges absolute rows by `(ad_id, minute_start)`, reconnects. **D47: the `EventSource` URL MUST carry `?cursor=<as_of_ingest_seq>`** from the snapshot just taken — `EventSource` cannot set a header, so without the query param every refresh silently drops the snapshot-to-subscribe gap | `src/web/stream.ts`, `src/web/store.ts` | Number ticks up live; kill the server mid-stream and restart → the client reconnects and the number is **not** double-counted | D§3.1, §5.5 | **HR1 HR2** |
