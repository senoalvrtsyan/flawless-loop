...
| *"the components and writings under 3 — The server, by role … is small and hard to read"* | **Real, and measurable.** That diagram's natural width is 3,582 px against GitHub's ~860 px cap, so it rendered at **scale 0.24 — 3.8 effective px**, the worst of the six by a wide margin. Seventeen nodes in five role groups do not fit one figure under the cap; **split into 3a (write) and 3b (read)**. A second pass then fixed the other half of the problem — width is what GitHub scales, **height is what makes you scroll**, and 3b was 1,424 px tall, readable across two screens. Both are now **under 700 px tall at 15.5–16 px**: 3a **888 × 669**, 3b **770 × 647**. The pass also found the diagram drew `agree.ts → replay.ts`, a call `scripts/agree.ts` deliberately never makes — it imports nothing | `22ba219` |
